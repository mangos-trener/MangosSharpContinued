...
{
    public WorldDatabase(ILogger<WorldDatabase> logger, MangosConfiguration configuration)
    {
        if (configuration == null || configuration.World == null || configuration.World.WorldDatabase == null)
        {
            logger.LogError("No connection string found for the account database!");
        }

        var connectionStringValues = configuration.World.WorldDatabase.Split(';', StringSplitOptions.RemoveEmptyEntries);

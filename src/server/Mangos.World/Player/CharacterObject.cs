...
public class CharacterObject : WS_Base.BaseUnit, IDisposable
{
    ...
    private readonly ILogger<CharacterObject> logger;
    ...
    private readonly NpcTextFactory npcTextFactory;
    ...
    private readonly WS_Handlers_Instance instanceHandler;
    private readonly CharManagementHandler charManagementHandler;
    private readonly WS_GameObjects gameObjectsService;
    ...
    public CharacterObject(
        ILogger<WS_Base.BaseUnit> baseLogger,
    ...
    : base(baseLogger, worldState, null, baseActiveSpellFactory, updateClassFactory)
    {
        this.logger = logger;
        ...
        this.npcTextFactory = npcTextFactory;
        ...
        Faction = 0;
        var Character_ = this;
        attackState = attackTimerFactory.Create(ref Character_);
        ...
        ManaRegenerationModifier = configuration.World.ManaRegenerationRate;
        LifeRegenerationModifier = configuration.World.HealthRegenerationRate;
        ManaRegenBonus = 0;
        ManaRegenPercent = 1f;
        ManaRegen = 0;
        ...
        RunBackSpeed = MangosGlobalConstants.UNIT_NORMAL_WALK_BACK_SPEED;
        SwimSpeed = MangosGlobalConstants.UNIT_NORMAL_SWIM_SPEED;
        SwimBackSpeed = MangosGlobalConstants.UNIT_NORMAL_SWIM_BACK_SPEED;
        TurnRate = MangosGlobalConstants.UNIT_NORMAL_TURN_RATE;
        charMovementFlags = 0;
        ZoneID = 0;
        AreaID = 0;
        ...
        UpdateMask = new BitArray(MangosGlobalConstants.FIELD_MASK_SIZE_PLAYER, defaultValue: false);
    ...
    public CharacterObject(
        ...
        AttackTimerFactory attackTimerFactory,
        CorpseObjectFactory corpseObjectFactory,
        ref WS_Network.ClientClass ClientVal,
        ulong GuidVal)
    : this(
        baseLogger,
        logger,
        ...
        corpseObjectFactory)
    {
        ...
        checked
        {
            ActiveSpells = new WS_Base.BaseActiveSpell[MangosGlobalConstants.MAX_AURA_EFFECTs - 1 + 1];
            client = ClientVal;
            GUID = GuidVal;
            ...
            characterDatabase.Query(string.Format("SELECT * FROM characters WHERE char_guid = {0}; UPDATE characters SET char_online = 1 WHERE char_guid = {0};", GUID), ref MySQLQuery);
            if (MySQLQuery.Rows.Count == 0)
            {
                logger.LogDebug("[{0}:{1}] Unable to get SQLDataBase info for character [GUID={2:X}]", client.IP, client.Port, GUID);
                Dispose();
                return;
            }
        ...
        if (database.CharRaces.ContainsKey(MySQLQuery.Rows[0].As<int>("char_race")))

        {
            Faction = database.CharRaces[MySQLQuery.Rows[0].As<int>("char_race")].FactionID;
            Model = Gender == Genders.GENDER_MALE
                ? database.CharRaces[MySQLQuery.Rows[0].As<int>("char_race")].ModelMale
                : database.CharRaces[MySQLQuery.Rows[0].As<int>("char_race")].ModelFemale;
        }
        if (Model == 0)
        {
        ...
        checked
        {
            ...
            if (tmp.Length > 0)
            {
                var currentTimestamp = Globals.Functions.GetTimestamp(DateAndTime.Now);
                ...
                for (var i3 = 0; i3 <= num2; i3++)
                {
                    ...
                    if (AuraSlot < 0 || AuraSlot >= MangosGlobalConstants.MAX_AURA_EFFECTs_VISIBLE || !WS_Spells.SPELLs.ContainsKey(AuraSpellID) || ActiveSpells[AuraSlot] != null)
                    {
                        continue;
                    }
                    int duration;
                    if (AuraExpire == 0)
                    {
                        duration = MangosGlobalConstants.SPELL_DURATION_INFINITE;
                    }
                    ...
                    ActiveSpells[AuraSlot] = baseActiveSpellFactory.Create(AuraSpellID, duration, null);
            ...
            Character_ = this;
            MySQLQuery.Clear();
            characterDatabase.Query($"SELECT * FROM corpse WHERE player = {GUID};", ref MySQLQuery);
            if (MySQLQuery.Rows.Count > 0)
            {
                corpseGUID = Conversions.ToULong(Operators.AddObject(MySQLQuery.Rows[0]["guid"], MangosGlobalConstants.GUID_CORPSE));
    ...
    public WS_Base.BaseUnit GetTarget
    {
        get
        {
            if (LegacyGlobalFunctions.GuidIsCreature(TargetGUID))
            {
                return worldState.WorldCreatures[TargetGUID];
            }
            if (LegacyGlobalFunctions.GuidIsPlayer(TargetGUID))
            {
                return worldState.Characters[TargetGUID];
            }
            return LegacyGlobalFunctions.GuidIsPet(TargetGUID)
                ? worldState.WorldCreatures[TargetGUID]
                : null;
        }
    }
    ...
    public bool IsMoving => (MangosGlobalConstants.movementFlagsMask & charMovementFlags) != 0;

    public bool IsTurning => (MangosGlobalConstants.TurningFlagsMask & charMovementFlags) != 0;

    public bool IsMovingOrTurning => (MangosGlobalConstants.movementOrTurningFlagsMask & charMovementFlags) != 0;
    ...
    public bool IsInCombat => inCombatWith.Count > 0 || checked(LegacyNativeMethods.TimeGetTime("") - lastPvpAction) < MangosGlobalConstants.DEFAULT_PVP_COMBAT_TIME;
    ...
    public bool AFK
    {
        ...
        set
        {
            if (value)
            {
                ...
                cluster.ClientSetChatFlag(client.Index, 1);
            }
            else
            {
                cPlayerFlags &= ~PlayerFlags.PLAYER_FLAGS_AFK;
                cluster.ClientSetChatFlag(client.Index, 0);
            }
        }
    }
    ...
    public bool DND
    {
        ...
        set
        {
            if (value)
            {
                ...
                cluster.ClientSetChatFlag(client.Index, 2);
            }
            else
            {
                cPlayerFlags &= ~PlayerFlags.PLAYER_FLAGS_DND;
    ...
    public bool GM
    {
        ...
        set
        {
            if (value)
            {
                ...
                cluster.ClientSetChatFlag(client.Index, 3);
            }
            else
            {
                cPlayerFlags &= ~PlayerFlags.PLAYER_FLAGS_GM;
    ...
    {
        characterDatabase.Update("INSERT INTO characters_honor (char_guid)  VALUES (" + Conversions.ToString(GUID) + ");");
    }
    ...
    public void HonorLoad()
    {
        ...
        characterDatabase.Query($"SELECT * FROM characters_honor WHERE char_guid = {GUID};", ref MySQLQuery);
        if (MySQLQuery.Rows.Count == 0)
        {
            logger.LogError("Unable to get SQLDataBase honor info for character [GUID={0:X}]", GUID);
            return;
        }
    ...
    public void AddToCombat(WS_Base.BaseUnit Unit)
    {
        ...
        {
            lastPvpAction = LegacyNativeMethods.TimeGetTime("");
        }
        else
        {
    ...
    public override bool CanSee(ref WS_Base.BaseObject objCharacter)
    {
        ...
        switch (objCharacter)
        {
            ...
            case GameObject _ when ((GameObject)objCharacter).Despawned:
                return false;
        }
        var distance = WS_Combat.GetDistance(this, objCharacter);
    ...
    {
        guidsForRemoving_Lock.AcquireWriterLock(MangosGlobalConstants.DEFAULT_LOCK_TIMEOUT);
            ...
            do
            {
                checked
                {
                    if (Items.ContainsKey(i))
                    {
                        ...
                        if (i < 19u)
                        {
                            SetUpdateFlag(260 + (i * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Items[i].ItemEntry);
                            SetUpdateFlag(268 + (i * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
                        }
                    }
                    else
                    {
                        ...
                        if (i < 19u)
                        {
                            SetUpdateFlag(260 + (i * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
                        }
                    }
                    i = (byte)unchecked((uint)(i + 1));
                }
    ...
    public void SendItemAndCharacterUpdate(ItemObject Item, int UPDATETYPE = 0)
    {
            ...
            do
            {
                checked
                {
                    if (Items.ContainsKey(j))
                    {
                        ...
                        if (j < 19u)
                        {
                            SetUpdateFlag(260 + (j * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Items[j].ItemEntry);
                            SetUpdateFlag(268 + (j * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Items[j].RandomProperties);
                        }
                    }
                    else
                    {
                        ...
                        if (j < 19u)
                        {
                            SetUpdateFlag(260 + (j * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
                        }
                    }
                    j = (byte)unchecked((uint)(j + 1));
                }
        ...
        do
        {
            checked
            {
                ...
                {
                    SetUpdateFlag(260 + (i * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Items[i].ItemEntry);
                    SetUpdateFlag(268 + (i * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Items[i].RandomProperties);
                }
                else
                {
                    SetUpdateFlag(260 + (i * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
    ...
    public void SendCharacterUpdate(bool toNear = true, bool notMe = false)
    {
        ...
        {
            var updateClass = updateClassFactory.Create(MangosGlobalConstants.FIELD_MASK_SIZE_PLAYER);
    ...
    public void FillAllUpdateFlags()
    {
        ...
        SetUpdateFlag(717, playerInitializator.XPTable[Level]);
        ...
        checked
        {
            ...
            var wS_Combat5 = combat;
            objCharacter = this;
            SetUpdateFlag(1108, wS_Combat5.GetBasePercentParry(ref objCharacter, 0));
            var b = (byte)MangosGlobalConstants.PLAYER_EXPLORED_ZONES_SIZE;
            byte m = 0;
            while (m <= (uint)b)
            {
            ...
            do
            {
                if (Items.ContainsKey(l))
                {
                    if (l < 19u)
                    {
                        SetUpdateFlag(260 + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Items[l].ItemEntry);
                        foreach (var Enchant in Items[l].Enchantments)
                        {
                            SetUpdateFlag(261 + (Enchant.Key * 3) + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Enchant.Value.ID);
                            SetUpdateFlag(262 + (Enchant.Key * 3) + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Enchant.Value.Charges);
                            SetUpdateFlag(263 + (Enchant.Key * 3) + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Enchant.Value.Duration);
                        }
                        SetUpdateFlag(268 + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Items[l].RandomProperties);
                    }
                ...
                else
                {
                    if (l < 19u)
                    {
                        SetUpdateFlag(260 + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
                        SetUpdateFlag(268 + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
                    }
            ...
            var b2 = (byte)(MangosGlobalConstants.MAX_AURA_EFFECTs_VISIBLE - 1);
            byte k = 0;
            while (k <= (uint)b2)
            {
            ...
            var b3 = (byte)(MangosGlobalConstants.MAX_AURA_EFFECT_FLAGs - 1);
            byte j = 0;
            while (j <= (uint)b3)
            {
            ...
            var b4 = (byte)(MangosGlobalConstants.MAX_AURA_EFFECT_LEVELSs - 1);
            byte i = 0;
            while (i <= (uint)b4)
            {
        ...
        checked
        {
            do
            {
                if (Items.ContainsKey(l))
                {
                    if (l < 19u)
                    {
                        Update.SetUpdateFlag(260 + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Items[l].ItemEntry);
                        Update.SetUpdateFlag(268 + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Items[l].RandomProperties);
                    }
                ...
                else
                {
                    if (l < 19u)
                    {
                        Update.SetUpdateFlag(260 + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
                        Update.SetUpdateFlag(268 + (l * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
                    }
            ...
            var b2 = (byte)(MangosGlobalConstants.MAX_AURA_EFFECT_FLAGs - 1);
            byte j = 0;
            while (j <= (uint)b2)
            {
            ...
            var b3 = (byte)(MangosGlobalConstants.MAX_AURA_EFFECT_LEVELSs - 1);
            byte i = 0;
            while (i <= (uint)b3)
            {
    ...
    {
        var packet = Globals.Functions.BuildChatMessage(logger, Sender.GUID, Message, msgType, (LANGUAGES)msgLanguage, WS_Handlers_Chat.GetChatFlag(Sender), ChannelName);
    ...
    public void CommandResponse(string Message)
    {
        ...
        {
            var packet = Globals.Functions.BuildChatMessage(logger, 2147483647uL, Msg, ChatMsg.CHAT_MSG_SYSTEM, LANGUAGES.LANG_GLOBAL);
            client.Send(ref packet);
            packet.Dispose();
        }
        logger.LogDebug("[{0}:{1}] SMSG_MESSAGECHAT", client.IP, client.Port);
    }

    public void SystemMessage(string Message)
    {
        Globals.Functions.SendMessageSystem(logger, client, Message);
    }
    ...
    public void ApplySpell(int SpellID)
    {
        if (WS_Spells.SPELLs.ContainsKey(SpellID))
        {
            ...
            {
                var spellInfo2 = WS_Spells.SPELLs[SpellID];
                WS_Base.BaseObject caster = this;
    ...
    public void ProhibitSpellSchool(int School, int Time)
    {
        ...
        checked
        {
            try
            {
                ...
                var curTime = Globals.Functions.GetTimestamp(DateAndTime.Now);
    ...
    public bool FinishSpell(CurrentSpellTypes SpellType, bool OK = false)
    {
        ...
        else
        {
            ...
            foreach (var tmpGO in array2)
            {
                ...
                {
                    gameObjectsService.Destroy(tmpGO);
    ...
    {
        if (Spells.ContainsKey(SpellID) || !WS_Spells.SPELLs.ContainsKey(SpellID))
        {
            return;
        }
        Spells.Add(SpellID, new WS_Spells.CharacterSpell(SpellID, 1, 0u, 0));
        characterDatabase.Update($"INSERT INTO characters_spells (guid, spellid, active, cooldown, cooldownitem) VALUES ({GUID},{SpellID},{1},{0},{0});");
        if (client == null)
        {
            return;
        }
        ...
        WS_Base.BaseUnit objCharacter = this;
        t.SetTarget_SELF(ref objCharacter);
        if (WS_Spells.SPELLs[SpellID].IsPassive)
        {
            int num;
        ...
        if (!WS_Spells.SPELLs[SpellID].CanStackSpellRank && WS_Spells.SpellChains.ContainsKey(SpellID) && Spells.ContainsKey(WS_Spells.SpellChains[SpellID]))
        {
            Spells[WS_Spells.SpellChains[SpellID]].Active = 0;
            characterDatabase.Update($"UPDATE characters_spells SET active = 0 WHERE guid = {GUID} AND spellid = {SpellID};");
            Packets.PacketClass packet = new(Opcodes.SMSG_SUPERCEDED_SPELL);
            try
            {
                packet.AddInt32(WS_Spells.SpellChains[SpellID]);
        ...
        checked
        {
            var maxSkill = (Level > playerInitializator.DEFAULT_MAX_LEVEL) ? (playerInitializator.DEFAULT_MAX_LEVEL * 5) : (Level * 5);
            switch (SpellID)
            {
                case 4036:
                    LearnSpell(3918);
    ...
    public void UnLearnSpell(int SpellID)
    {
        if (Spells.ContainsKey(SpellID))
        {
            ...
            characterDatabase.Update($"DELETE FROM characters_spells WHERE guid = {GUID} AND spellid = {SpellID};");
    ...
    public void LearnSkill(int SkillID, short Current = 1, short Maximum = 1)
    {
        checked
        {
            ...
            {
                var num = (short)MangosGlobalConstants.PLAYER_SKILL_INFO_SIZE;
                short i = 0;
                while (i <= num && SkillsPositions.ContainsValue(i))
                {
                    i = (short)unchecked(i + 1);
                }
                if (i > MangosGlobalConstants.PLAYER_SKILL_INFO_SIZE)
                {
                    return;
                }
    ...
    {
        if (SkillID != 0 && Skills[SkillID].Current < Skills[SkillID].Maximum && (Skills[SkillID].Current / (double)Skills[SkillID].Maximum) - SpeedMod < WorldState.Rnd.NextDouble())
        {
    ...
    public void SetLevel(byte SetToLevel)
    {
        ...
        if (Level <= (uint)SetToLevel)
        {
            ...
            checked
            {
                ...
                {
                    TotalXp += playerInitializator.XPTable[i];
                }
    ...
    {
        if (Ammount <= 0 || Level >= playerInitializator.DEFAULT_MAX_LEVEL)
        {
            return;
        }
        checked
        {
            ...
            do
            {
                if (XP >= playerInitializator.XPTable[Level])
                {
                    XP -= playerInitializator.XPTable[Level];
                    Level++;
                    GroupUpdateFlag |= 64u;
                    cluster.ClientUpdate(client.Index, (uint)ZoneID, Level);
                    ...
                    var wS_Player_Initializator = playerInitializator;
                    var objCharacter = this;
            ...
            while (XP >= playerInitializator.XPTable[Level] && Level < playerInitializator.DEFAULT_MAX_LEVEL);
            if (XP > playerInitializator.XPTable[Level])
            {
                XP = playerInitializator.XPTable[Level];
            }
    ...
    {
        var tmpItem = itemObjectFactory.Create(ItemEntry, GUID);
        ...
        checked
        {
            if (srcBag == 0)
            {
                if (srcSlot < 23u)
                {
                    if (srcSlot < 19u)
                    {
                        SetUpdateFlag(260 + (srcSlot * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
                    }
                ...
                characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {MangosGlobalConstants.ITEM_SLOT_NULL}, item_bag = {MangosGlobalConstants.ITEM_BAG_NULL} WHERE item_guid = {Items[srcSlot].GUID - MangosGlobalConstants.GUID_ITEM};");
                if (Destroy)
                {
            ...
            {
                characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {MangosGlobalConstants.ITEM_SLOT_NULL}, item_bag = {MangosGlobalConstants.ITEM_BAG_NULL} WHERE item_guid = {Items[srcBag].Items[srcSlot].GUID - MangosGlobalConstants.GUID_ITEM};");
                if (Destroy)
                {
    ...
    public void ItemREMOVE(ulong itemGuid, bool Destroy, bool Update)
    {
        ...
        do
        {
            checked
            {
                ...
                {
                    characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {MangosGlobalConstants.ITEM_SLOT_NULL}, item_bag = {MangosGlobalConstants.ITEM_BAG_NULL} WHERE item_guid = {Items[slot].GUID - MangosGlobalConstants.GUID_ITEM};");
                    if (slot < 23u)
                    {
                        if (slot < 19u)
                        {
                            SetUpdateFlag(260 + (slot * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), 0);
                        }
                        Dictionary<byte, ItemObject> items;
                        byte key;
            ...
            checked
            {
                if (Items.ContainsKey(bag))
                {
                    ...
                    while (slot2 <= (uint)b)
                    {
                        ...
                        {
                            characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {MangosGlobalConstants.ITEM_SLOT_NULL}, item_bag = {MangosGlobalConstants.ITEM_BAG_NULL} WHERE item_guid = {Items[bag].Items[slot2].GUID - MangosGlobalConstants.GUID_ITEM};");
                            if (Destroy)
                            {
    ...
    {
        var Slot = MangosGlobalConstants.ITEM_SLOT_NULL;
        checked
        {
            var OldestTime = (int)Globals.Functions.GetTimestamp(DateAndTime.Now);
            var OldestSlot = MangosGlobalConstants.ITEM_SLOT_NULL;
            byte i = 69;
            do
            {
                if (!Items.ContainsKey(i) || BuyBackTimeStamp[(byte)unchecked((uint)(i - 69))] == 0)
                {
                    if (Slot == MangosGlobalConstants.ITEM_SLOT_NULL)
                    {
                        Slot = i;
                    }
                }
            ...
            if (Slot == MangosGlobalConstants.ITEM_SLOT_NULL)
            {
                if (OldestSlot != MangosGlobalConstants.ITEM_SLOT_NULL)
                {
                    return;
                }
    ...
    public bool ItemADD_AutoBag(ref ItemObject Item, byte dstBag)
    {
        checked
        {
            ...
            else if (Items.ContainsKey(dstBag))
            {
                ...
                {
                    logger.LogDebug("{0} - {1} - {2}", Items[dstBag].ItemInfo.ObjectClass, Items[dstBag].ItemInfo.SubClass, Item.ItemInfo.BagFamily);
                    var wS_Items = WorldServiceLocator.WSItems;
                    objCharacter = this;
    ...
    public bool ItemSETSLOT(ref ItemObject Item, byte dstBag, byte dstSlot)
    {
        ...
        checked
        {
            ...
            {
                Items[dstSlot] = Item;
                characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {dstSlot}, item_bag = {GUID}, item_stackCount = {Item.StackCount} WHERE item_guid = {Item.GUID - MangosGlobalConstants.GUID_ITEM};");
                SetUpdateFlag(486 + (dstSlot * 2), Item.GUID);
                if (dstSlot < 19u)
                {
                    SetUpdateFlag(260 + (dstSlot * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Item.ItemEntry);
                    SetUpdateFlag(268 + (dstSlot * MangosGlobalConstants.PLAYER_VISIBLE_ITEM_SIZE), Item.RandomProperties);
            ...
            else
            {
                ...
                characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {dstSlot}, item_bag = {Items[dstBag].GUID}, item_stackCount = {Item.StackCount} WHERE item_guid = {Item.GUID - MangosGlobalConstants.GUID_ITEM};");
            }
            if (client != null)
            {
    ...
    public InventoryChangeFailure ItemCANEQUIP(ItemObject Item, byte dstBag, byte dstSlot)
    {
        ...
        catch (Exception ex)
        {
            ...
            var err = ex;
            logger.LogError("[{0}:{1}] Unable to equip item. {2} {3}", client.IP, client.Port, Environment.NewLine, err.ToString());
    ...
    public void ItemSPLIT(byte srcBag, byte srcSlot, byte dstBag, byte dstSlot, int Count)
    {
        ...
        checked
        {
            ...
            else if (dstItem == null)
            {
                ...
                var tmpItem = itemObjectFactory.Create(srcItem.ItemEntry, GUID, Count);
                dstItem = tmpItem;
                tmpItem.Save();
                ItemSETSLOT(ref tmpItem, dstBag, dstSlot);
    ...
    public void ItemSWAP(byte srcBag, byte srcSlot, byte dstBag, byte dstSlot)
    {
        ...
        checked
        {
            ...
            IL_06f5:
                ...
                characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {dstSlot}, item_bag = {GUID} WHERE item_guid = {Items[dstSlot].GUID - MangosGlobalConstants.GUID_ITEM};");
                if (Items[srcBag].Items.ContainsKey(srcSlot))
                {
                    characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {srcSlot}, item_bag = {Items[srcBag].GUID} WHERE item_guid = {Items[srcBag].Items[srcSlot].GUID - MangosGlobalConstants.GUID_ITEM};");
                }
                goto end_IL_0080;
            IL_0ab8:
                SendItemAndCharacterUpdate(Items[dstBag]);
                characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {dstSlot}, item_bag = {Items[dstBag].GUID} WHERE item_guid = {Items[dstBag].Items[dstSlot].GUID - MangosGlobalConstants.GUID_ITEM};");
                if (Items.ContainsKey(srcSlot))
                {
                    characterDatabase.Update($"UPDATE characters_inventory SET item_slot = {srcSlot}, item_bag = {GUID} WHERE item_guid = {Items[srcSlot].GUID - MangosGlobalConstants.GUID_ITEM};");
                }
                goto end_IL_0080;
            IL_0ec5:
            ...
            catch (Exception ex)
            {
                ProjectData.SetProjectError(ex);
                var err = ex;
                logger.LogDebug("[{0}:{1}] Unable to swap items. {2}{3}", client.IP, client.Port, Environment.NewLine, err.ToString());
                ProjectData.ClearProjectError();
            }
            finally
            {
    ...
    public ItemObject ItemGETByGUID(ulong GUID)
    {
        ...
        return srcSlot == MangosGlobalConstants.ITEM_SLOT_NULL ? null : ItemGET(srcBag, srcSlot);
    }
    ...
    public byte ItemGetSLOTBAG(ulong GUID, ref byte bag)
    {
        ...
        while (bag <= 22u);
        bag = MangosGlobalConstants.ITEM_SLOT_NULL;
        return MangosGlobalConstants.ITEM_SLOT_NULL;
    }
    ...
    public void UpdateAddItemStats(ref ItemObject Item, byte slot)
    {
        ...
        checked
        {
            ...
            do
            {
                if (Item.ItemInfo.Spells[i].SpellID > 0 && WS_Spells.SPELLs.ContainsKey(Item.ItemInfo.Spells[i].SpellID))
                {
                    var SpellInfo = WS_Spells.SPELLs[Item.ItemInfo.Spells[i].SpellID];
    ...
    {
        if (!WS_Creatures.NPCTexts.ContainsKey(TextID))
        {
            var tmpText = npcTextFactory.Create(TextID);
        }
        Packets.PacketClass response = new(Opcodes.SMSG_NPC_TEXT_UPDATE);
        try
        {
            response.AddInt32(TextID);
            if (WS_Creatures.NPCTexts[TextID].Count == 0)
            {
                response.AddInt32(0);
                response.AddString(WS_Creatures.NPCTexts[TextID].TextLine1[0]);
                response.AddString(WS_Creatures.NPCTexts[TextID].TextLine2[0]);
            }
            else
            {
                var i = 0;
                do
                {
                    response.AddSingle(WS_Creatures.NPCTexts[TextID].Probability[i]);
                    response.AddString(WS_Creatures.NPCTexts[TextID].TextLine1[i]);
                    if (Operators.CompareString(WS_Creatures.NPCTexts[TextID].TextLine2[i], "", TextCompare: false) == 0)
                    {
                        response.AddString(WS_Creatures.NPCTexts[TextID].TextLine1[i]);
                    }
                    else
                    {
                        response.AddString(WS_Creatures.NPCTexts[TextID].TextLine2[i]);
                    }
                    response.AddInt32(WS_Creatures.NPCTexts[TextID].Language[i]);
                    response.AddInt32(WS_Creatures.NPCTexts[TextID].EmoteDelay1[i]);
                    response.AddInt32(WS_Creatures.NPCTexts[TextID].Emote1[i]);
                    response.AddInt32(WS_Creatures.NPCTexts[TextID].EmoteDelay2[i]);
                    response.AddInt32(WS_Creatures.NPCTexts[TextID].Emote2[i]);
                    response.AddInt32(WS_Creatures.NPCTexts[TextID].EmoteDelay3[i]);
                    response.AddInt32(WS_Creatures.NPCTexts[TextID].Emote3[i]);
                    i = checked(i + 1);
                }
                while (i <= 7);
            }
    ...
    public void Teleport(float posX, float posY, float posZ, float posO, int map)
    {
        ...
        logger.LogInformation("World: Player Teleport: X[{0}], Y[{1}], Z[{2}], O[{3}]", posX, posY, posZ, posO);
        charMovementFlags = 0;
        ...
        orientation = posO;
        var Character = this;
        cellUpdater.MoveCell(ref Character);
        Character = this;
        cellUpdater.UpdateCell(ref Character);
        client.Character.ZoneID = maps.AreaTable[maps.GetAreaFlag(posX, posY, checked((int)client.Character.MapID))].Zone;
    }

    public void Transfer(float posX, float posY, float posZ, float ori, int map)
    {
        logger.LogInformation("World: Player Transfer: X[{0}], Y[{1}], Z[{2}], O[{3}], MAP[{4}]", posX, posY, posZ, ori, map);
        ...
        checked
        {
            ...
            var cellUpdater = this.cellUpdater;
            var Character = this;
            cellUpdater.RemoveFromWorld(ref Character);
            if (OnTransport is not null and WS_Transports.TransportObject @object)
            {
                var obj = @object;
            ...
            worldServer.CLSWorldServer.ClientTransfer(client.Index, posX, posY, posZ, ori, map);
        }
    }

    public void ZoneCheck()
    {
        var ZoneFlag = maps.GetAreaFlag(positionX, positionY, checked((int)MapID));
        if (!maps.AreaTable.ContainsKey(ZoneFlag))
        {
            logger.LogWarning("Zone Flag {0} does not exist.", ZoneFlag);
            return;
        }
        AreaID = maps.AreaTable[ZoneFlag].ID;
        ZoneID = maps.AreaTable[ZoneFlag].Zone == 0
            ? maps.AreaTable[ZoneFlag].ID
            : maps.AreaTable[ZoneFlag].Zone;
        GroupUpdateFlag |= 128u;
        if (maps.AreaTable[ZoneFlag].IsCity())
        {
            if ((cPlayerFlags & PlayerFlags.PLAYER_FLAGS_RESTING) == 0 && Level < playerInitializator.DEFAULT_MAX_LEVEL)
            {
        ...
        if (maps.AreaTable[ZoneFlag].IsSanctuary())
        {
        ...
        if (maps.AreaTable[ZoneFlag].IsArena())
        {
        ...
        var tArea = maps.AreaTable[ZoneFlag];
        var objCharacter = this;
    ...
    public void ZoneCheckInstance()
    {
        ...
        AreaID = maps.AreaTable[ZoneFlag].ID;
        if (maps.AreaTable[ZoneFlag].Zone == 0)
        {
            ZoneID = ZoneID == 0 ? maps.AreaTable[ZoneFlag].ID : maps.AreaTable[ZoneFlag].Zone;
        }
        GroupUpdateFlag |= 128u;
        if (maps.AreaTable[ZoneFlag].IsCity())
        {
    ...
    public void SetMoveRoot()
    {
        ...
        logger.LogDebug("[{0}:{1}] SMSG_FORCE_MOVE_ROOT", client.IP, client.Port);
    }
    ...
    public void SetMoveUnroot()
    {
        ...
        logger.LogDebug("[{0}:{1}] SMSG_FORCE_MOVE_UNROOT", client.IP, client.Port);
    }
    ...
    public void HandleDrowning(object state)
    {
        checked
        {
            try
            {
                if (positionZ > maps.GetWaterLevel(positionX, positionY, (int)MapID) - 1.6)
                {
            ...
            {
                ProjectData.SetProjectError(ex);
                logger.LogError("Error at {methodName}: {message}", nameof(HandleDrowning), ex.Message);
                underWaterTimer?.Dispose();
                underWaterTimer = null;
                ProjectData.ClearProjectError();
            }
    ...
    {
        if (database.FactionInfo[FactionID].VisibleID > -1)
        {
            Reputation[database.FactionInfo[FactionID].VisibleID].Value = 0;
            if (Reputation[database.FactionInfo[FactionID].VisibleID].Flags == 0)
            {
                Reputation[database.FactionInfo[FactionID].VisibleID].Flags = 1;
            }
        }
    }
    ...
    {
        if (!database.FactionTemplatesInfo.ContainsKey(FactionID) || !database.FactionTemplatesInfo.ContainsKey(Faction))
        {
            return TReaction.NEUTRAL;
        }
        if (((database.FactionTemplatesInfo[FactionID].enemyMask == 0L && database.FactionTemplatesInfo[FactionID].friendMask == 0L && database.FactionTemplatesInfo[FactionID].enemyFaction1 == 0) & (database.FactionTemplatesInfo[FactionID].enemyFaction2 == 0)) && database.FactionTemplatesInfo[FactionID].enemyFaction3 == 0 && database.FactionTemplatesInfo[FactionID].enemyFaction4 == 0)
        {
            return TReaction.NEUTRAL;
        }
        if (((database.FactionTemplatesInfo[FactionID].enemyMask == 0L && database.FactionTemplatesInfo[FactionID].friendMask == 0L && database.FactionTemplatesInfo[FactionID].enemyFaction1 != Faction) & (database.FactionTemplatesInfo[FactionID].enemyFaction2 != Faction)) && database.FactionTemplatesInfo[FactionID].enemyFaction3 != Faction && database.FactionTemplatesInfo[FactionID].enemyFaction4 != Faction)
        {
            return TReaction.NEUTRAL;
        }
        if ((database.FactionTemplatesInfo[FactionID].enemyMask & 1uL) != 0)
        {
            return TReaction.HOSTILE;
        }
        if (database.FactionTemplatesInfo[FactionID].friendFaction1 == Faction || database.FactionTemplatesInfo[FactionID].friendFaction2 == Faction || database.FactionTemplatesInfo[FactionID].friendFaction3 == Faction || database.FactionTemplatesInfo[FactionID].friendFaction4 == Faction)
        {
            return TReaction.FIGHT_SUPPORT;
        }
        if ((database.FactionTemplatesInfo[FactionID].friendMask & database.FactionTemplatesInfo[Faction].ourMask) != 0)
        {
            return TReaction.FIGHT_SUPPORT;
        }
        if (database.FactionTemplatesInfo[FactionID].enemyFaction1 == Faction || database.FactionTemplatesInfo[FactionID].enemyFaction2 == Faction || database.FactionTemplatesInfo[FactionID].enemyFaction3 == Faction || database.FactionTemplatesInfo[FactionID].enemyFaction4 == Faction)
        {
            return TReaction.HOSTILE;
        }
        if ((database.FactionTemplatesInfo[FactionID].enemyMask & database.FactionTemplatesInfo[Faction].ourMask) != 0)
        {
            return TReaction.HOSTILE;
        }
        return GetReputation(database.FactionTemplatesInfo[FactionID].FactionID) switch
        {
    ...
    {
        if (!database.FactionTemplatesInfo.ContainsKey(FactionTemplateID))
        {
            return 3;
        }
        var FactionID = database.FactionTemplatesInfo[FactionTemplateID].FactionID;
        if (!database.FactionInfo.ContainsKey(FactionID))
        {
            return 3;
        }
        if (database.FactionInfo[FactionID].VisibleID == -1)
        ...
        checked
        {
            var points = Globals.Functions.HaveFlag((uint)database.FactionInfo[FactionID].flags[0], (byte)((int)Race - 1)) ? database.FactionInfo[FactionID].rep_stats[0] : (Globals.Functions.HaveFlag((uint)database.FactionInfo[FactionID].flags[1], (byte)((int)Race - 1)) ? database.FactionInfo[FactionID].rep_stats[1] : (Globals.Functions.HaveFlag((uint)database.FactionInfo[FactionID].flags[2], (byte)((int)Race - 1)) ? database.FactionInfo[FactionID].rep_stats[2] : (Globals.Functions.HaveFlag((uint)database.FactionInfo[FactionID].flags[3], (byte)((int)Race - 1)) ? database.FactionInfo[FactionID].rep_stats[3] : 0)));
            if (Reputation[database.FactionInfo[FactionID].VisibleID].Flags > 0)
            {
                points += Reputation[database.FactionInfo[FactionID].VisibleID].Value;
            }
            return points;
        }
    }
    ...
    public void SetReputation(int FactionID, int Value)
    {
        ...
        checked
        {
            Reputation[database.FactionInfo[FactionID].VisibleID].Value += Value;
            if ((Reputation[database.FactionInfo[FactionID].VisibleID].Flags & 1) == 0)
            {
                Reputation[database.FactionInfo[FactionID].VisibleID].Flags = Reputation[database.FactionInfo[FactionID].VisibleID].Flags | 1;
            }
            if (client != null)
            {
                ...
                try
                {
                    packet.AddInt32(Reputation[database.FactionInfo[FactionID].VisibleID].Flags);
                    packet.AddInt32(database.FactionInfo[FactionID].VisibleID);
                    packet.AddInt32(Reputation[database.FactionInfo[FactionID].VisibleID].Value);
                    client.Send(ref packet);
                }
                finally
                {
    ...
    public override void Die(ref WS_Base.BaseUnit Attacker)
    {
        ...
        {
            if (LegacyGlobalFunctions.GuidIsPlayer(uGuid) && worldState.Characters.ContainsKey(uGuid))
            {
                worldState.Characters[uGuid].RemoveFromCombat(this);
            }
        }
        inCombatWith.Clear();
        if (IsInDuel)
        {
            ...
            var Loser = this;
            WS_Spells.DuelComplete(worldState, gameObjectsService, ref duelPartner, ref Loser);
            return;
        }
        checked
        {
            ...
            var Loser = this;
            repopTimer = new WS_PlayerHelper.TRepopTimer(worldState, ref Loser, maps, misc);
            cDynamicFlags = 32;
            cUnitFlags = 8;
            SetUpdateFlag(22, 0);
            SetUpdateFlag(46, cUnitFlags);
    ...
    public override void DealDamage(int Damage, WS_Base.BaseUnit Attacker = null)
    {
        ...
        if (Attacker != null)
        {
            ...
            {
                if (worldState.WorldCreatures.ContainsKey(cGUID) && worldState.WorldCreatures[cGUID].aiScript != null && worldState.WorldCreatures[cGUID].IsGuard && !worldState.WorldCreatures[cGUID].IsDead && !worldState.WorldCreatures[cGUID].aiScript.InCombat && !inCombatWith.Contains(cGUID) && GetReaction(worldState.WorldCreatures[cGUID].Faction) == TReaction.FIGHT_SUPPORT && WS_Combat.GetDistance(worldState.WorldCreatures[cGUID], this) <= worldState.WorldCreatures[cGUID].AggroRange(this))
                {
                    worldState.WorldCreatures[cGUID].aiScript.OnGenerateHate(ref Attacker, Damage);
                }
            }
        }
        GroupUpdateFlag |= 2u;
    ...
    public void Logout(object StateObj = null)
    {
        ...
        if (this is CharacterObject _character)
        {
            if (repopTimer != null)
            {
                repopTimer?.Dispose();
                repopTimer = null;
                var myCorpse = corpseObjectFactory.Create(ref _character);
                myCorpse?.AddToWorld();
                myCorpse?.Save();
            }
            ...
            if (DuelPartner != null)
            {
                ...
                {
                    ref var duelPartner = ref DuelPartner;
                    WS_Spells.DuelComplete(worldState, gameObjectsService, ref duelPartner, ref _character);
                }
                else if (worldState.WorldGameObjects.ContainsKey(DuelArbiter))
                {
                    gameObjectsService.Destroy(worldState.WorldGameObjects[DuelArbiter]);
                }
            }
            ...
            try
            {
                ...
                logger.LogDebug("[{0}:{1}] SMSG_LOGOUT_COMPLETE", client.IP, client.Port);
                client.Character = null;
                logger.LogInformation("Character {0} logged off.", Name);
                client?.Delete();
                client = null;
            }
            finally
    ...
    public void Login()
    {
        instanceHandler.InstanceMapEnter(this, maps);
        SetOnTransport();
        if (MapID != LoginMap)
        {
            logger.LogDebug("Spawned on wrong map [{0}], transferring to [{1}].", LoginMap, MapID);
        ...
        catch (Exception ex2)
        {
            ...
            var ex = ex2;
            logger.LogCritical("Failed loading maps at character logging in.{0}{1}", Environment.NewLine, ex.ToString());
            ProjectData.ClearProjectError();
        }

        // TODO: ensure reference integrity
        ref var reference = ref client;
        var character = this;
        playerHelper.SendBindPointUpdate(ref reference, ref character);
        playerHelper.Send_SMSG_SET_REST_START(ref reference, ref character);
        playerHelper.SendTutorialFlags(ref reference, ref character);

        SendProficiencies();

        playerHelper.SendInitialSpells(ref reference, ref character);
        playerHelper.SendFactions(ref reference, ref character);
        playerHelper.SendActionButtons(ref reference, ref character);
        playerHelper.SendInitWorldStates(ref reference, ref character);
        ...
        var cellUpdater = this.cellUpdater;
        character = this;
        cellUpdater.AddToWorld(ref character);
        Globals.Functions.SendTimeSyncReq(logger, ref client);
        UpdateAuraDurations();
        FullyLoggedIn = true;
        UpdateManaRegen();
    }
    ...
    public void SetOnTransport()
    {
        ...
        logger.LogDebug("Spawning on transport.");
        ...
        checked
        {
            if (decimal.Compare(new decimal(TransportGUID), 0m) > 0)
            {
                ...
                {
                    OnTransport = worldState.WorldTransports[TransportGUID];
                    var transportObject = worldState.WorldTransports[TransportGUID];
                    WS_Base.BaseUnit Unit = this;
                ...
                else if (LegacyGlobalFunctions.GuidIsTransport(TransportGUID))
                {
                    if (worldState.WorldGameObjects.ContainsKey(TransportGUID))
                    {
                        OnTransport = worldState.WorldGameObjects[TransportGUID];
                        transportX = positionX;
                        transportY = positionY;
                        transportZ = positionZ;
                        positionX = OnTransport.positionX;
                    ...
                    {
                        logger.LogCritical("Spawning new transport!");
                        var cGUID = TransportGUID - MangosGlobalConstants.GUID_TRANSPORT;
                        DataRow row = null;
                        var newGameobject = gameObjectFactory.Create(cGUID, row);
                        gameObjectsService.AddToWorld(newGameobject);
                ...
                {
                    logger.LogCritical("Character logging in on non-existant transport [{0}].", TransportGUID - MangosGlobalConstants.GUID_MO_TRANSPORT);
                    var allGraveYards = worldState.GraveyardsService;
                    var Character = this;
    ...
    public void SendProficiencies()
    {
        ...
        checked
        {
            ...
            Globals.Functions.SendProficiency(logger, ref client, 4, ProficiencyFlags);
            ProficiencyFlags = 0;
            if (HaveSpell(196))
            {
                ProficiencyFlags++;
            ...
            Globals.Functions.SendProficiency(logger, ref client, 2, ProficiencyFlags);
        }
    }
    ...
    protected virtual void Dispose(bool disposing)
    {
        if (!_disposedValue)
        {
            characterDatabase.Update($"DELETE FROM characters_inventory WHERE item_bag = {GUID} AND item_slot >= {69} AND item_slot <= {79}");
            underWaterTimer?.Dispose();
            if (repopTimer != null)
            {
                repopTimer.Dispose();
                repopTimer = null;
                var Character = this;
                var myCorpse = corpseObjectFactory.Create(ref Character);
                myCorpse.Save();
                myCorpse.AddToWorld();
            }
            ...
            worldState.Characters.Remove(GUID);
            ...
            logger.LogInformation("Character {0} disposed.", Name);
                    ...
                    foreach (var slot in Items[i].Items)
                    {
                        if (slot.Value.ItemInfo.ObjectClass == ITEM_CLASS.ITEM_CLASS_PROJECTILE && slot.Value.ItemInfo.SubClass == AmmoType)
                        {
                            ...
                            if (charManagementHandler.CanUseAmmo(ref objCharacter, slot.Value.ItemEntry) == InventoryChangeFailure.EQUIP_ERR_OK)
                            {
                                ...
                                AmmoDPS = worldState.ItemDatabase[AmmoID].Damage[0].Minimum;
                                var wS_Combat = combat;
                                objCharacter = this;
    ...
    public void SaveCharacter()
    {
        ...
        checked
        {
            ...
            for (var i = 0; i <= num; i++)
            {
                if (ActiveSpells[i] != null && (ActiveSpells[i].SpellDuration == MangosGlobalConstants.SPELL_DURATION_INFINITE || ActiveSpells[i].SpellDuration > 10000))
                {
                    var expire = 0L;
    ...
    public bool TalkAddQuest(ref WS_QuestInfo Quest)
    {
        ...
        checked
        {
            do
            {
                ...
                {
                    worldState.QuestsService.CreateQuest(ref TalkQuests[i], ref Quest);
                    ...
                    characterDatabase.Update($"INSERT INTO characters_quests (char_guid, quest_id, quest_status) VALUES ({GUID}, {TalkQuests[i].ID}, {questState});");
    ...
    public bool TalkDeleteQuest(byte QuestSlot)
    {
        ...
        checked
        {
            ...
            characterDatabase.Update($"DELETE  FROM characters_quests WHERE char_guid = {GUID} AND quest_id = {TalkQuests[QuestSlot].ID};");
    ...
    public bool TalkCompleteQuest(byte QuestSlot)
    {
        ...
        checked
        {
            ...
            characterDatabase.Update($"UPDATE characters_quests SET quest_status = -1 WHERE char_guid = {GUID} AND quest_id = {TalkQuests[QuestSlot].ID};");
            TalkQuests[QuestSlot] = null;
            return true;
        }
    }
    ...
    public bool TalkUpdateQuest(byte QuestSlot)
    {
        ...
        checked
        {
            ...
            {
                var tmpTimer = (int)(TalkQuests[QuestSlot].TimeEnd - Globals.Functions.GetTimestamp(DateAndTime.Now));
            }
    ...
    public bool TalkCanAccept(ref WS_QuestInfo Quest)
    {
        ...
        characterDatabase.Query($"SELECT quest_status FROM characters_quests WHERE char_guid = {GUID} AND quest_id = {Quest.ID} LIMIT 1;", ref DBResult);
    ...
    {
        DataTable q = new();
        characterDatabase.Query($"SELECT quest_id FROM characters_quests WHERE char_guid = {GUID} AND quest_status = -1 AND quest_id = {QuestID};", ref q);

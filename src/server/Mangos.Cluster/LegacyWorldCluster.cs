...
public class LegacyWorldCluster : IClusterContext
{
    private readonly MangosConfiguration _mangosConfiguration;
    private readonly WsDbcLoad _dbcLoad;
    private readonly WcNetwork network;
    private readonly WsDbcDatabase database;

    public LegacyWorldCluster(MangosConfiguration mangosConfiguration, WsDbcLoad dbcLoad, WcNetwork network, WsDbcDatabase database)
    {
    ...
    // System Things...
    public BaseWriter Log { get; set; } = new ColoredConsoleWriter();
    ...
    public async Task StartAsync()
    {
        ...
        int returnValues;
        returnValues = database.GetAccountDatabase().Connect();
        ...
        database.GetAccountDatabase().Update("SET NAMES 'utf8';");
        returnValues = database.GetCharacterDatabase().Connect();
        if (returnValues > (int)SQL.ReturnState.Success)   // Ok, An error occurred
        {
            Console.WriteLine("[{0}] An SQL Error has occurred", Strings.Format(DateAndTime.TimeOfDay, "hh:mm:ss"));
            Console.WriteLine("*************************");
        ...
        database.GetCharacterDatabase().Update("SET NAMES 'utf8';");
        returnValues = database.GetWorldDatabase().Connect();
        if (returnValues > (int)SQL.ReturnState.Success)   // Ok, An error occurred
        {
            Console.WriteLine("[{0}] An SQL Error has occurred", Strings.Format(DateAndTime.TimeOfDay, "hh:mm:ss"));
            Console.WriteLine("*************************");
        ...
        database.GetWorldDatabase().Update("SET NAMES 'utf8';");
        await _dbcLoad.InitializeInternalDatabaseAsync(this);
        //handlers.IntializePacketHandlers();
        if (Common.Legacy.Globals.LegacyGlobalFunctions.CheckRequiredDbVersion(database.GetAccountDatabase(), ServerDb.Realm) == false)         // Check the Database version, exit if its wrong
        {
            if (true)
            {
                Console.WriteLine("*************************");
        ...
        if (Common.Legacy.Globals.LegacyGlobalFunctions.CheckRequiredDbVersion(database.GetCharacterDatabase(), ServerDb.Character) == false)         // Check the Database version, exit if its wrong
        {
            if (true)
            {
                Console.WriteLine("*************************");
        ...
        if (Common.Legacy.Globals.LegacyGlobalFunctions.CheckRequiredDbVersion(database.GetWorldDatabase(), ServerDb.World) == false)         // Check the Database version, exit if its wrong
        {
            if (true)
            {
                Console.WriteLine("*************************");
    ...
    public void WaitConsoleCommand()
    {
        ...
        int varList;
        while (!network.WorldServer.MFlagStopListen)
        {
            try
            {
                tmp = Log.ReadLine();
                            ...
                            case "shutdown":
                                {
                                    ...
                                    network.WorldServer.MFlagStopListen = true;
                                    break;
                                }

...
public class WcHandlersAuth
{
    ...
    public WcHandlersAuth(LegacyWorldCluster cluster,
    ...
    {
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_AUTH_SESSION [{2}]", client.IP, client.Port, client.Account);
        Thread.Sleep(500);
    ...
    public void On_CMSG_AUTH_SESSION(PacketClass packet, ClientClass client)
    {
        ...
        foreach (var tmpClientEntry in cluster.ClienTs)
        {
        ...
        database.GetAccountDatabase().Query(query, ref result);
        ...
        {
            cluster.Log.WriteLine(LogType.USER, "[{0}:{1}] AUTH_UNKNOWN_ACCOUNT: Account not in DB!", client.IP, client.Port);
        ...
        if (cluster.ClienTs.Count > mangosConfiguration.Cluster.ServerPlayerLimit && client.Access <= AccessLevel.Player)
        {
        ...
        packet.Data = zip.DeCompress(decompressBuffer);
        packet.Offset = 0;
    ...
    public void On_CMSG_UPDATE_ACCOUNT_DATA(PacketClass packet, ClientClass client)
    {
        try
        {
            ...
            cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_UPDATE_ACCOUNT_DATA [ID={2} Size={3}]", client.IP, client.Port, dataId, uncompressedSize);
            if (dataId > 7L)
            {
                return;
            }
        ...
        {
            cluster.Log.WriteLine(LogType.FAILED, "Error while updating account data.{0}", Constants.vbCrLf + e);
        }
    }
    ...
    public void On_CMSG_REQUEST_ACCOUNT_DATA(PacketClass packet, ClientClass client)
    {
        ...
        packet.GetInt16();
        var dataId = packet.GetUInt32();
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_REQUEST_ACCOUNT_DATA [ID={2}]", client.IP, client.Port, dataId);
        if (dataId > 7L)
        {
            return;
        }
    ...
    {
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHAR_ENUM", client.IP, client.Port);
        ...
        try
        {
            database.GetAccountDatabase().Query(string.Format("SELECT id FROM account WHERE username = '{0}';", client.Account), ref mySqlQuery);
            accountId = mySqlQuery.Rows[0].As<int>("id");
            mySqlQuery.Clear();
            database.GetCharacterDatabase().Query(string.Format("SELECT * FROM characters WHERE account_id = '{0}' ORDER BY char_guid;", accountId), ref mySqlQuery);
            ...
            for (int i = 0, loopTo = mySqlQuery.Rows.Count - 1; i <= loopTo; i++)
            {
                ...
                database.GetCharacterDatabase().Query(string.Format("SELECT COUNT(*) FROM corpse WHERE player = {0};", mySqlQuery.Rows[i]["char_guid"]), ref deadMySqlQuery);
                ...
                database.GetCharacterDatabase().Query(string.Format("SELECT modelid, level, entry FROM character_pet WHERE owner = '{0}';", mySqlQuery.Rows[i]["char_guid"]), ref petQuery);
                ...
                if (petQuery.Rows.Count > 0)
                {
                    ...
                    database.GetWorldDatabase().Query(string.Format("SELECT family FROM creature_template WHERE entry = '{0}'", petQuery.Rows[0]["entry"]), ref petFamilyQuery);
                ...
                var characterDb = database.GetCharacterDatabase().SQLDBName;
                var worldDb = database.GetWorldDatabase().SQLDBName;
                database.GetCharacterDatabase().Query(string.Format("SELECT item_slot, displayid, inventorytype FROM " + characterDb + ".characters_inventory, " + worldDb + ".item_template WHERE item_bag = {0} AND item_slot <> 255 AND entry = item_id  ORDER BY item_slot;", guid), ref itemsMySqlQuery);
        ...
        {
            cluster.Log.WriteLine(LogType.FAILED, "[{0}:{1}] Unable to enum characters. [{2}]", client.IP, client.Port, e.Message);
        ...
        client.Send(response);
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_CHAR_ENUM", client.IP, client.Port);
    }

    public void On_CMSG_CHAR_DELETE(PacketClass packet, ClientClass client)
    {
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHAR_DELETE", client.IP, client.Port);
        ...
        try
        {
            ...
            database.GetAccountDatabase().Query(string.Format("SELECT id FROM account WHERE username = \"{0}\";", client.Account), ref q);
            if (q.Rows.Count == 0)
            {
                return;
            }

            database.GetCharacterDatabase().Query(string.Format("SELECT char_guid FROM characters WHERE account_id = \"{0}\" AND char_guid = \"{1}\";", q.Rows[0]["id"], guid), ref q);
            if (q.Rows.Count == 0)
            {
                response.AddInt8((byte)AuthResult.WOW_FAIL_BANNED);
                client.Send(response);
                GlobalFunctions.Ban_Account(cluster, database, client.Account, "Packet Manipulation/Character Deletion");
                client.Delete();
                return;
            }

            q.Clear();
            database.GetCharacterDatabase().Query(string.Format("SELECT item_guid FROM characters_inventory WHERE item_bag = {0};", guid), ref q);
            foreach (DataRow row in q.Rows)
            {
                // DONE: Delete items
                database.GetCharacterDatabase().Update(string.Format("DELETE FROM characters_inventory WHERE item_guid = \"{0}\";", row.As<string>("item_guid")));
                // DONE: Delete items in bags
                database.GetCharacterDatabase().Update(string.Format("DELETE FROM characters_inventory WHERE item_bag = \"{0}\";", row.As<ulong>("item_guid") + MangosGlobalConstants.GUID_ITEM));
            }

            database.GetCharacterDatabase().Query(string.Format("SELECT item_guid FROM characters_inventory WHERE item_owner = {0};", guid), ref q);
            q.Clear();
            database.GetCharacterDatabase().Query(string.Format("SELECT mail_id FROM characters_mail WHERE mail_receiver = \"{0}\";", guid), ref q);
            foreach (DataRow row in q.Rows)
            {
                // TODO: Return mails?
                // DONE: Delete mails
                database.GetCharacterDatabase().Update(string.Format("DELETE FROM characters_mail WHERE mail_id = \"{0}\";", row.As<string>("mail_id")));
                // DONE: Delete mail items
                database.GetCharacterDatabase().Update(string.Format("DELETE FROM mail_items WHERE mail_id = \"{0}\";", row.As<string>("mail_id")));
            }

            database.GetCharacterDatabase().Update(string.Format("DELETE FROM characters WHERE char_guid = \"{0}\";", guid));
            database.GetCharacterDatabase().Update(string.Format("DELETE FROM characters_honor WHERE char_guid = \"{0}\";", guid));
            database.GetCharacterDatabase().Update(string.Format("DELETE FROM characters_quests WHERE char_guid = \"{0}\";", guid));
            database.GetCharacterDatabase().Update(string.Format("DELETE FROM character_social WHERE guid = '{0}' OR friend = '{0}';", guid));
            database.GetCharacterDatabase().Update(string.Format("DELETE FROM characters_spells WHERE guid = \"{0}\";", guid));
            database.GetCharacterDatabase().Update(string.Format("DELETE FROM petitions WHERE petition_owner = \"{0}\";", guid));
            database.GetCharacterDatabase().Update(string.Format("DELETE FROM auctionhouse WHERE auction_owner = \"{0}\";", guid));
            database.GetCharacterDatabase().Update(string.Format("DELETE FROM characters_tickets WHERE char_guid = \"{0}\";", guid));
            database.GetCharacterDatabase().Update(string.Format("DELETE FROM corpse WHERE guid = \"{0}\";", guid));
            q.Clear();
            database.GetCharacterDatabase().Query(string.Format("SELECT guild_id FROM guilds WHERE guild_leader = \"{0}\";", guid), ref q);
            if (q.Rows.Count > 0)
            {
                database.GetCharacterDatabase().Update(string.Format("UPDATE characters SET char_guildid = 0, char_guildrank = 0, char_guildpnote = '', charguildoffnote = '' WHERE char_guildid = \"{0}\";", q.Rows[0]["guild_id"]));
                database.GetCharacterDatabase().Update(string.Format("DELETE FROM guild WHERE guild_id = \"{0}\";", q.Rows[0]["guild_id"]));
            }
        ...
        client.Send(response);
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] SMSG_CHAR_DELETE [{2:X}]", client.IP, client.Port, guid);
    }
    ...
    public void On_CMSG_CHAR_RENAME(PacketClass packet, ClientClass client)
    {
        ...
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHAR_RENAME [{2}:{3}]", client.IP, client.Port, guid, name);
        var errCode = (byte)ATLoginFlags.AT_LOGIN_RENAME;

        // DONE: Check for existing name
        DataTable q = new();
        database.GetCharacterDatabase().Query(string.Format("SELECT char_name FROM characters WHERE char_name LIKE \"{0}\";", name), ref q);
        if (q.Rows.Count > 0)
        {
        ...
        {
            database.GetCharacterDatabase().Update(string.Format("UPDATE characters SET char_name = \"{1}\", force_restrictions = 0 WHERE char_guid = {0};", guid, name));
        }
    ...
    public void On_CMSG_CHAR_CREATE(PacketClass packet, ClientClass client)
    {
        packet.GetInt16();
        var name = packet.GetString();
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_CHAR_CREATE [{2}]", client.IP, client.Port, name);
        ...
        try
        {
            if (network.WorldServer.Worlds.ContainsKey(0U))
            {
                result = network.WorldServer.Worlds[0U].ClientCreateCharacter(client.Account, name, race, classe, gender, skin, face, hairStyle, hairColor, facialHair, outfitId);
            }
            else if (network.WorldServer.Worlds.ContainsKey(1U))
            {
                result = network.WorldServer.Worlds[1U].ClientCreateCharacter(client.Account, name, race, classe, gender, skin, face, hairStyle, hairColor, facialHair, outfitId);
            }
        }
        catch (Exception ex)
        {
            result = (int)CharResponse.CHAR_CREATE_ERROR;
            cluster.Log.WriteLine(LogType.FAILED, "[{0}:{1}] Character creation failed!{2}{3}", client.IP, client.Port, Constants.vbCrLf, ex.ToString());
        }
    ...
    public void On_CMSG_PLAYER_LOGIN(PacketClass packet, ClientClass client)
    {
        ...
        var guid = packet.GetUInt64();
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_PLAYER_LOGIN [0x{2:X}]", client.IP, client.Port, guid);
        if (client.Character is null)
        {
            client.Character = new WcHandlerCharacter.CharacterObject(guid, client, cluster, database, social, network, guild, channels, group);
        }
        ...
        if (network.WorldServer.InstanceCheck(client, client.Character.Map))
        {
        ...
        {
            cluster.Log.WriteLine(LogType.FAILED, "[{0:000000}] Unable to login: WORLD SERVER DOWN", client.Index);
            client.Character.Dispose();
            ...
            {
                cluster.Log.WriteLine(LogType.FAILED, "[{0:000000}] Unable to login: {1}", client.Index, ex.ToString());
                client.Character.Dispose();
                client.Character = null;
    ...
    {
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] CMSG_PLAYER_LOGOUT", client.IP, client.Port);
    ...
    {
        cluster.Log.WriteLine(LogType.DEBUG, "[{0}:{1}] MSG_MOVE_WORLDPORT_ACK", client.IP, client.Port);
        try
        {
            if (!network.WorldServer.InstanceCheck(client, client.Character.Map))
            {
        ...
        {
            cluster.Log.WriteLine(LogType.CRITICAL, "{0}", ex.ToString());
        }
    }
}
